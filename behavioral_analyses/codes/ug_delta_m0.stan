...
data {
  ...
  int<lower=1, upper=T_max> Tsubj; //vector of trial num per sub
  vector[T_max] real offer; // N by T matrix, N row of subject, T col of trials
  vector[T_max] int<lower=0, upper=1> accept;
}
...
// Declare all parameters as vectors for vectorizing
  ...
  real alpha_pr;  // alpha: Envy (sensitivity to norm prediction error), N of sd
  real tau_pr;    // tau: Inverse temperature, N of sd
  real ep_pr;     // ep: Norm adaptation rate, N of sd
