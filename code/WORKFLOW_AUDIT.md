...
| RT companion-event construction | Established for source; production partially verified | Among 6,655 responded trials, 931 (13.99%) lack a companion `event_RT` row. All 805 responded first trials of blocks lack that row; the other 126 omissions are every non-first trial in both runs for sub-143, whose files contain no `event_RT` rows. The two tracked sub-143 event TSVs are byte-identical to OpenNeuro ds003745 snapshot 2.0.2 (commit `88d8d458`) and each retains all 72 substantive response rows. In the current `srndna-ug` server tree, 92/94 runs have current RT EVs and nonconstant retained design columns; sub-143 has incomplete L1/L2 shells. All tracked L3 templates nevertheless include sub-143 using paths under the distinct legacy `srndna-ultimatum` root. Audit that template-named root before proposing any rerun. |
