...
| RT companion-event construction | Established for source; production partially verified | Among 6,655 responded trials, 931 (13.99%) lack a companion `event_RT` row. All 805 responded first trials of blocks lack that row; the other 126 omissions are every non-first trial in both runs for sub-143, whose files contain no `event_RT` rows. The two tracked sub-143 event TSVs are byte-identical to OpenNeuro ds003745 snapshot 2.0.2 (commit `88d8d458`) and each retains all 72 substantive response rows. On the production server, 92/94 runs have current RT and RT-pmod EVs with row counts matching the source plus retained, nonconstant design columns. Both sub-143 FSFs exist, but their current EV files and `design.mat` files do not. Trace downstream inclusion before proposing any rerun. |
