...
| RT companion-event construction | Established in curated BIDS; production pending | Among 6,655 responded trials, 931 (13.99%) lack a companion `event_RT` row. All 805 responded first trials of blocks lack that row; the other 126 omissions are every non-first trial in both runs for sub-143, whose files contain no `event_RT` rows. The substantive task-event rows remain present. The production 3-column EVs and FEAT designs must be inspected before inferring the modeled consequence or proposing a rerun. |
