...
| RT companion-event construction | Established for source; production partially verified | Among 6,655 responded trials, 931 (13.99%) lack a companion `event_RT` row. All 805 responded first trials of blocks lack that row; the other 126 omissions are every non-first trial in both runs for sub-143, whose files contain no `event_RT` rows. The two tracked sub-143 event TSVs are byte-identical to OpenNeuro ds003745 snapshot 2.0.2 (commit `88d8d458`) and each retains all 72 substantive response rows. The current `srndna-ug` derivative mirror has 92/94 RT designs and incomplete sub-143 shells. The template-named legacy `srndna-ultimatum` root retains sub-143 activation, DMN, and ECN L2 copes 4, 6, and 7 from August 2021. Rerun the RT design audit against that legacy root and recover its submitted L3 design before proposing any regeneration. |
